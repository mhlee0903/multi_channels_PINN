...
def data_valid(pair_dataset, table_prot0, table_mol2vec):
    # get protein 
...
def train_generator(batch_size, pair_dataset, table_prot0, table_mol2vec):
...
class transfer_(Callback):
    def __init__(self, model, path_w, path_csv, task, full_learning='',
        ...
        with tf.device('/cpu:0'):
            self.model_ = MODEL_dic[args.MODEL_str](dropout_value=0.5 , ecfp=0)
...
########### Model Build! ##############
with tf.device('/cpu:0'):
    model= MODEL_dic[args.MODEL_str](dropout_value=0.5 , ecfp=0)
